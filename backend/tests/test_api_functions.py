...
from db import chat, job, message
...
def test_get_job_returns_front_end_dict_for_a_real_job():
    created = job.create("pull_model", "gemma3:270m")

    data = functions.py_get_job(created.id)
...
def test_get_job_returns_none_for_an_unknown_id():
    assert functions.py_get_job(9999) is None


def test_list_jobs_lists_every_job():
    job.create("pull_model", "a")
    job.create("pull_model", "b")

    assert [j["payload"] for j in functions.py_list_jobs()] == ["a", "b"]
