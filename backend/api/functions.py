...
import logging
import constants
from db import message, chat, job
...
@eel.expose
def py_get_job(job_id):
    found = job.get(job_id)
    return None if found is None else found.toFrontEnd()

@eel.expose
def py_list_jobs():
    return [jobitem.toFrontEnd() for jobitem in job.list()]
